...
TEST_CASE( "Unscented Kalman Filter", "[UKF]" ) {
  ...
  SECTION("Predict Radar Measurement")
  {
    ...
    cout << "Predicted z:" << endl << ukf.z_pred_ << endl;
    REQUIRE(ukf.z_pred_.cols() == z_pred_exp.cols());
    REQUIRE(ukf.z_pred_.rows() == z_pred_exp.rows());
    REQUIRE((ukf.z_pred_ - z_pred_exp).norm() < 1e-4);

    MatrixXd S_exp = MatrixXd(ukf.n_z_, ukf.n_z_);
    S_exp << 0.0946171, -0.000139448, 0.00407016,
        -0.000139448, 0.000617548, -0.000770652,
        0.00407016, -0.000770652, 0.0180917;
    cout << "Covariance S:" << endl
         << ukf.S_ << endl;
    REQUIRE(ukf.S_.cols() == S_exp.cols());
    REQUIRE(ukf.S_.rows() == S_exp.rows());
    REQUIRE((ukf.S_ - S_exp).norm() < 1e-4);
  }
  ...
  SECTION("Update") 
  {
    ...
    ukf.Zsig_ << 6.1190, 6.2334, 6.1531, 6.1283, 6.1143, 6.1190, 6.1221, 6.1190, 6.0079, 6.0883, 6.1125, 6.1248, 6.1190, 6.1188, 6.12057,
    ...
    ukf.S_ << 0.0946171, -0.000139448, 0.00407016,
        -0.000139448, 0.000617548, -0.000770652,
        0.00407016, -0.000770652, 0.0180917;
